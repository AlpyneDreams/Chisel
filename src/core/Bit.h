...
// From DXVK
  ...
  #define CHISEL_ARCH_X86
  #if defined(__x86_64__) || defined(_M_X64)
    #define CHISEL_ARCH_X86_64
  #endif
#elif defined(__aarch64__) || defined(_M_ARM64)
  #define CHISEL_ARCH_ARM64
#else
#error "Unknown CPU Architecture"
#endif

#ifdef CHISEL_ARCH_X86
  #ifndef _MSC_VER
...
namespace chisel::bit {
  ...
  inline uint32_t tzcnt(uint32_t n) {
    ...
    #elif defined(CHISEL_ARCH_X86) && (defined(__GNUC__) || defined(__clang__))
  ...
  inline uint32_t tzcnt(uint64_t n) {
    #if defined(CHISEL_ARCH_X86_64) && defined(_MSC_VER) && !defined(__clang__)
    return (uint32_t)_tzcnt_u64(n);
    #elif defined(CHISEL_ARCH_X86_64) && defined(__BMI__)
    return __tzcnt_u64(n);
    #elif defined(CHISEL_ARCH_X86_64) && (defined(__GNUC__) || defined(__clang__))
    uint64_t res;
    uint64_t tmp;
    asm (
      "tzcnt %2, %0;"
  ...
  bool bcmpeq(const T* a, const T* b) {
    ...
    #if defined(CHISEL_ARCH_X86) && (defined(__GNUC__) || defined(__clang__) || defined(_MSC_VER))
...
#if (defined(__GNUC__) || defined(__clang__)) && !defined(__BMI__) && defined(CHISEL_ARCH_X86)
        uint32_t res;
        asm ("tzcnt %1,%0"
        : "=r" (res)
        : "r" (m_mask)
